...
export default function CreateRidePage() {
  const [form, setForm] = useState<RideForm>(() => ({
    ...
    minPeople: "2", maxPeople: "6", speedLevel: 2, bikeRequirement: "", description: "", agreed: true,
  }));
  ...
  const validate = (): string | null => {
    if (!form.title.trim()) return "请填写约骑标题";
    ...
    if (!form.agreed) return "请先同意安全免责声明";
    return null;
  };
  ...
  const publish = useCallback(async () => {
    ...
    try {
      const payload: CreateRidePayload = {
        ...
        rules: { safety_agreed: true },
      };
      setCreated(await rideService.create(payload));
    } catch (requestError) {
  ...
  return <View className="create-ride">
    <View className="create-ride__group"><Text className="create-ride__group-title">基本信息</Text>
      <View className="create-ride__field"><Text>约骑标题</Text><Input value={form.title} maxlength={30} placeholder="如：南山周末轻松骑" onInput={(event) => update("title", event.detail.value)} /><Text className="create-ride__count">{form.title.length}/30</Text></View>
    ...
    <View className="create-ride__agreement" onClick={() => update("agreed", !form.agreed)}><Text className={form.agreed ? "create-ride__check create-ride__check--checked" : "create-ride__check"}>✓</Text><Text>我已阅读并同意安全免责声明，骑行风险自行承担</Text></View>
    <View className="create-ride__footer"><View className={submitting ? "create-ride__submit create-ride__submit--disabled" : "create-ride__submit"} onClick={() => void publish()}>{submitting ? "发布中…" : "发布约骑"}</View></View>
