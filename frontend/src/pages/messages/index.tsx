...
export default function Messages() {
  ...
  return <View className="messages">
    ...
    {loading ? <><Skeleton /><Skeleton /></> : items.length ? items.map((item) => <View key={item.id} className={item.is_read ? "messages__item" : "messages__item messages__item--unread"} onClick={() => void open(item)}><View className={item.is_read ? "messages__icon" : "messages__icon messages__icon--unread"}>{item.type === 6 ? "◆" : item.is_read ? "○" : "●"}</View><View className="messages__body"><Text>{item.title}</Text><Text>{item.content}</Text><Text>{formatRelativeTime(item.created_at)}</Text></View>{item.unread_dot ? <View className="messages__dot" /> : null}</View>) : <Empty text="暂时没有助手通知" />}
  </View>;
}
