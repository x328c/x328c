...
export default function MyRides() {
  ...
  const load = async (nextType = type) => {
    ...
    setFailed(false);
    try {
      const list = (await rideService.mine(nextType)).list;
  ...
  return <View className="my-list">
    ...
    {loading ? <Skeleton /> : failed ? <Empty text="加载失败，请下拉刷新或稍后重试" /> : items.length ? items.map((ride) => <View key={ride.id} className="my-list__ride"><RideCard ride={ride} onClick={() => Taro.navigateTo({ url: `/pages/rides/detail/index?id=${ride.id}` })} />{historical(ride) ? <View className="my-list__relaunch" onClick={() => Taro.navigateTo({ url: `/pages/rides/create/index?relaunchId=${ride.id}` })}>再次发起同行</View> : null}</View>) : <Empty text="暂无同行" />}
  </View>;
}
