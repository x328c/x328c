...
export default function Settings() {
  ...
  return <View className="settings">
    ...
    {section("通知", <>{toggle("同行微信推送", "ride_notifications")}{toggle("活动微信推送", "activity_notifications")}{toggle("系统微信推送", "system_notifications")}<View className="settings__row" onClick={() => Taro.openSetting()}><Text>微信系统通知权限</Text><Text>前往系统设置 ›</Text></View></>)}
    ...
    {section("存储与版本", <><View className="settings__row" onClick={() => void clearCache()}><Text>清理非关键缓存</Text><Text>›</Text></View><View className="settings__row"><Text>版本</Text><Text>V2.1 · {ENV} · {__TAB_VARIANT__ === "5" ? "5 Tab" : "4 Tab"}</Text></View></>)}
