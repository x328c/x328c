...
export default function RegulationListPage() {
  ...
  return <View className="regulations-page">
    <View className="regulations-page__hero"><Text className="regulations-page__title">法规检索</Text><Text className="regulations-page__subtitle">官方信息结构化索引，不提供法律结论</Text><Input className="regulations-page__search" value={keyword} maxlength={100} placeholder="搜索标题、文号、机构或标签" confirmType="search" onInput={(event) => setKeyword(event.detail.value)} onConfirm={search} /></View>
    ...
    {state === "disabled" ? <StatePanel type="disabled" title="法规检索暂未开放" description="路线与约骑功能仍可正常使用" actionText="返回约骑" onAction={() => Taro.switchTab({ url: "/pages/index/index" })} /> : null}
