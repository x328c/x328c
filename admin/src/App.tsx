...
export default function App() { return <Suspense fallback={<div className="page-loading">加载中…</div>}><Routes><Route path="/login" element={<LoginPage />} /><Route element={<Guard><AdminLayout /></Guard>}><Route path="/" element={<DashboardPage />} /><Route path="/rides" element={<RidesPage />} /><Route path="/routes" element={<RoutesPage />} /><Route path="/regulations" element={<RegulationsPage />} /><Route path="/feature-flags" element={<FeatureFlagsPage />} /><Route path="/maintenance" element={<MaintenancePage />} /><Route path="/v21-governance" element={<V21GovernancePage />} /><Route path="/users" element={<UsersPage />} /><Route path="/reports" element={<ReportsPage />} /></Route><Route path="*" element={<Navigate to="/" replace />} /></Routes></Suspense>; }
