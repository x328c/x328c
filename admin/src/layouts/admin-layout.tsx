...
const names: Record<string, string> = { '/': '数据概览', '/rides': '约骑管理', '/routes': '路线管理', '/regulations': '法规管理', '/forum': '论坛治理', '/activities': '活动管理', '/users': '用户管理', '/reports': '举报处理', '/feature-flags': '功能开关', '/maintenance': '集成运维' };

export function AdminLayout() {
  const location = useLocation(); const navigate = useNavigate();
  const { admin, logout } = useAuthStore();
  const visibleMenu = admin?.role === 2 ? menu.filter((item) => item?.key === '/regulations') : menu;
